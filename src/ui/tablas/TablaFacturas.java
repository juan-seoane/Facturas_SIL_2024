...
private Image icon;
    ...
    public TablaFacturas(@SuppressWarnings("rawtypes") Vector<? extends Vector> vector, Vector<String> columnas) throws NullPointerException, IOException{
    ...
    */
    ...
    public boolean actualizarModelo(Vector<? extends Vector> facturas, int sel) throws NullPointerException, IOException{
        ...
        tablafacturas.setModel(modelo);
            ...
            for (int i = 0; i<Config.getConfig(Controlador.usuario).getNombresColumnasStandard().length;i++)
            {
                TableColumn columna = tablafacturas.getColumn(Config.getConfig(Controlador.usuario).getNombresColumnasStandard()[i]);
                if (Config.getConfig(Controlador.usuario).getAnchoColumnasStandard()[i]!=0)
                {
                    columna.setMaxWidth((int)Config.getConfig(Controlador.usuario).getAnchoColumnasStandard()[i]);
                    columna.setMinWidth((int)Config.getConfig(Controlador.usuario).getAnchoColumnasStandard()[i]);
                }
            }
